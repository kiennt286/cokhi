...
const KnowledgePreviewSection = () => {
  const posts = knowledgePosts.slice(0, 10);
    ...
    <section className="py-16 bg-white">
        <div className="flex items-end justify-between gap-4 mb-8 flex-wrap">
          <div>
            ...
            <h2 className="font-montserrat font-extrabold text-2xl">Kinh Nghiệm Gia Công CNC</h2>
          </div>
          <Link to="/kien-thuc" className="font-montserrat text-sm font-semibold text-[#ED3524] hover:underline">
            Xem tất cả bài viết
          </Link>
        </div>
        ...
        <div className="relative">
              ...
              <SwiperSlide key={post.slug} className="h-auto">
                <article className="border border-gray-200 bg-[#f8fafc] overflow-hidden h-full flex flex-col">
                  <Link to={`/kien-thuc/${post.slug}`}>
                    {post.cover ? (
                      <img src={post.cover} alt={post.title} className="block w-full aspect-[1/1] object-cover" />
                    ) : (
                  ...
                  <div className="p-4 flex flex-col flex-1">
                    ...
                    <Link to={`/kien-thuc/${post.slug}`} className="inline-block mt-auto  text-sm font-semibold text-[#ED3524] hover:underline">
                      Đọc thêm
                    </Link>
                  </div>
                </article>
          ...
          <div ref={paginationRef} className="knowledge-preview-pagination mt-3 flex justify-center" />
        </div>
    </section>
  );
};
